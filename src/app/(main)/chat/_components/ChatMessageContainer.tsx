...
function ChatMessageContainer({ roomId }: { roomId: number }) {
  ...
  return (
    <div className="flex items-center justify-center w-dvh h-[calc(100dvh-60px)] lg:h-[calc(100vh-70px)]">
      <div className="w-full max-w-[630px] flex flex-col h-full">
        <div className="relative mb-3 w-full h-full max-h-[100px]">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center py-2">
            ...
            <p className="hidden md:block text-sm text-black">실시간으로 대화가 진행되고 있어요!</p>
            <p className="hidden md:block text-sm text-black">소비와 관련된 대화를 나누어 주세요</p>
          </div>
        </div>
